...
float L; float statetop, statebott;
...
void setup() 
{
  ...
  lcd.setCursor(0,2);lcd.print("Tank:");
  lcd.setCursor(11,2);lcd.print("Fan:");
  lcd.setCursor(0,3);lcd.print("Eva Pump:");
}
 ...
 if(Vout>0)
  {
    ...
    lcd.setCursor(11,3);lcd.print("LO");
  }
  else
  {
    digitalWrite(relay1, LOW);
...
void WaterTank()
{
  ...
  if (statetop == HIGH && statebott == HIGH) 
  { 
    Serial.println( "Valve Aktif");
    digitalWrite(relay4, HIGH);
    TV = 1;
    lcd.setCursor(5,2);lcd.print("HI");
 
  } 
  else if(statetop == LOW )
  { 
    Serial.println( "Valve Mati" );
    digitalWrite(relay4, LOW);
    TV = 0;
    lcd.setCursor(5,2);lcd.print("LO");
  }
  else
  {
    Serial.println( "Valve Mati" );
...
{ soilmoisture();
  Dht22();
  // Print a message to the LCD_______________________
  
  int hh = h; int tt = t;int sm =soilval;
  
  lcd.setCursor(5,0); lcd.print(tt);//(kolom,baris)
  lcd.setCursor(17,0);lcd.print(soilval);
  lcd.setCursor(5,1);lcd.print(hh);
}
...
{ 
  LCD();
  WaterTank();
  hasilCooling(); PompaCooling();
  hasilSoil();    PompaSoil();
  lamp();
  
  //Serial Communication ke Esp32_____
  soilmoisture();
  Dht22();
  soilval = soilval + 50;// 0
